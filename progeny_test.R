...
refs <- "0/0"
alts <- c("1/1","2/2","3/3","2/1","1/2","1/3","3/1","2/3","3/2")
hets <- c("1/0","0/1","2/0","0/2","3/0","0/3")
missing  <- "./."
...
#write.csv(geno_short_mat, "justincase2.csv", row.names=TRUE)
justin <- read.csv(file = "/home/sbusato/justincase2.csv", check.names = FALSE)
rownames(justin) <- justin[,1]
colnames(justin)
geno_short_mat <- as.matrix(justin[,-1])

sums_per_SNP <- rowSums(geno_short_mat, na.rm = TRUE)

SNP_rem_allzero <- sums_per_SNP[sums_per_SNP==0]
SNP_rem_allone <- sums_per_SNP[sums_per_SNP==4929]


stdev_per_SNP <- apply(geno_short_mat, 1, function(x) as.numeric(sd(x, na.rm = TRUE)))
#hist(stdev_per_SNP)
SNP_rem_lowSTD <- stdev_per_SNP[stdev_per_SNP < 0.2]
...
sample_rem <- data.frame(missing_per_sample[missing_per_sample>=50000]) #117 samples
colnames(sample_rem) <- c("countmissing")
SNP_rem <- data.frame(missing_per_snp[missing_per_snp>=1000]) #8562 SNPs
colnames(SNP_rem) <- c("countmissing")
SNP_zero <- data.frame(SNP_rem_allzero) #6276 SNPs
colnames(SNP_zero) <- c("countmissing")
SNP_one <- data.frame(SNP_rem_allone)
colnames(SNP_one) <- c("countmissing")
SNP_std <- data.frame(SNP_rem_lowSTD)
colnames(SNP_std) <- c("countmissing")

#NEW APPROACH 01032023 -- 96% call rate & coverage
#4928 samples --> 4730 is 96% / 198 is 4%
#392999 SNPs --> 377279 is 96% / 15720 is 4%

sample_rem <- data.frame(missing_per_sample[missing_per_sample>=15721]) #166 samples
colnames(sample_rem) <- c("countmissing")
SNP_rem <- data.frame(missing_per_snp[missing_per_snp>=199]) #37763 SNPs
colnames(SNP_rem) <- c("countmissing")
SNP_zero <- data.frame(SNP_rem_allzero) #6276 SNPs
colnames(SNP_zero) <- c("countmissing")
SNP_one <- data.frame(SNP_rem_allone) #19 SNPs
colnames(SNP_one) <- c("countmissing")

##optional (for test2) -- remove SNPs with SD lower than 2 
SNP_std <- data.frame(SNP_rem_lowSTD)
colnames(SNP_std) <- c("countmissing")
...
geno_short_mat_filt <- geno_short_mat_filt[rownames(geno_short_mat_filt) %nin% rownames(SNP_zero),]
geno_short_mat_filt <- geno_short_mat_filt[rownames(geno_short_mat_filt) %nin% rownames(SNP_one),]
 #removed 166 hybrids and 42910 SNPs when no SD

geno_short_mat_filt <- geno_short_mat_filt[rownames(geno_short_mat_filt) %nin% rownames(SNP_std),]
dim(geno_short_mat) - dim(geno_short_mat_filt)#removed 166 hybrids and 98375 SNPs when SD>0.2 constrain



#####PROGENY CALCULATION

same_ds_short <- bettermeta[bettermeta$match_ds_short == TRUE, ]
diff_ds_short <- bettermeta[bettermeta$match_ds_short == FALSE, ]

geno_same <- geno_short_mat_filt[,colnames(geno_short_mat_filt) %in% same_ds_short$hybrid]
geno_diff <- geno_short_mat_filt[,colnames(geno_short_mat_filt) %in% diff_ds_short$hybrid]


gs.mat.inv <- t(geno_same)
colnames(gs.mat.inv) <- rownames(geno_same)

G <- Gmatrix(gs.mat.inv, method = "VanRaden", ploidy = 2)
png(filename = "heatmap_same_df_short.png", height = 960, width = 960)
hmap <- heatmap(G)
dev.off()

png(filename = "hist_samedfshort_diag.png")
hist_diag <- hist(G[row(G)==col(G)])
png(filename = "hist_samedfshort_nondiag.png")
hist_nondiag <- hist(G[row(G)!=col(G)])
dev.off()
...
gs.mat.inv <- t(geno_diff)
colnames(gs.mat.inv) <- rownames(geno_diff)

G <- Gmatrix(gs.mat.inv, method = "VanRaden", ploidy = 2)
png(filename = "heatmap_diff_df_short.png", height = 960, width = 960)
hmap <- heatmap(G)
dev.off()

png(filename = "hist_diffdfshort_diag.png")
hist_diag <- hist(G[row(G)==col(G)])
dev.off()
png(filename = "hist_diffdfshort_nondiag.png")
hist_nondiag <- hist(G[row(G)!=col(G)])
dev.off()
...
##exploring some clustering that's odd 
rowclustering <- hmap$rowInd
colclustering <- hmap$colInd
samplenames <- rownames(G)

clust <- data.frame(samplenames, rowclustering)
colnames(clust) <- c("hybrid","order")

clust2 <- clust[clust$order < 500, ]
clust2_meta <- bettermeta[bettermeta$hybrid %in% clust2$hybrid,]
clust2_meta %>% group_by(dataset_trim_p2) %>% summarise(count=n())

sample_rem <- clust2_meta[clust2_meta$dataset_trim_p1=="Assembly" | clust2_meta$dataset_trim_p2=="Assembly",]
sample_rem <- sample_rem$hybrid

geno_diff_filt <- geno_diff[,colnames(geno_diff) %nin% sample_rem]

gs.mat.inv <- t(geno_diff_filt)
colnames(gs.mat.inv) <- rownames(geno_diff_filt)

G <- Gmatrix(gs.mat.inv, method = "VanRaden", ploidy = 2)
png(filename = "heatmap_diff_df_short_NOASSEMBLY.png", height = 960, width = 960)
hmap <- heatmap(G)
dev.off()

png(filename = "hist_diffdfshort_diag_NOASSEMBLY.png")
hist_diag <- hist(G[row(G)==col(G)])
dev.off()
png(filename = "hist_diffdfshort_nondiag_NOASSEMBLY.png")
hist_nondiag <- hist(G[row(G)!=col(G)])
dev.off()
...
###trying -- LD to filter variants
LD_keeps <- read.delim("/home/sbusato/plink.prune.in", sep = "\t", header = FALSE)
LD_keeps <- LD_keeps$V1

LD_filt <- geno_short_mat_filt[rownames(geno_short_mat_filt) %in% LD_keeps,]
dim(geno_short_mat_filt) - dim(LD_filt) #removed 325989 SNPs, 24100 remaining
                                        #with SD constraint: removed 284325 SNPs, 10299 remaining

gs.mat.inv <- t(LD_filt)
colnames(gs.mat.inv) <- rownames(LD_filt)

##saving the inverse matrix for the model
#write.csv(gs.mat.inv, "5_Geno_LD_filt_noNAs_01042023.csv", row.names=TRUE)


G <- Gmatrix(gs.mat.inv, method = "VanRaden", ploidy = 2)
png(filename = "heatmap_all_LDfilt_SDfilt.png", height = 960, width = 960)
hmap <- heatmap(G)
dev.off()

png(filename = "hist_all_diag_LDfilt_SDfilt.png")
hist_diag <- hist(G[row(G)==col(G)])
dev.off()
png(filename = "hist_all_nondiag_LDfilt_SDfilt.png")
hist_nondiag <- hist(G[row(G)!=col(G)])
dev.off()
...
### see which ones are NA on the remaining ones 

write.csv(LD_filt, file = "LD_filt_withNAs.csv")
LD_filt[is.na(LD_filt)] <- 0

write.csv(LD_filt, file = "LD_filt_SD_filt_noNAs.csv")
...
#A matrix - expected relationship matrix -- does not work, why???
parents <- colnames(geno_short_mat_filt)
parent1 <- ""
parent2 <- ""
for (i in 1:length(parents)){
  parent1[i] <- strsplit(parents[i], split = "/")[[1]][1]
...
families$ID <- paste("IND",as.character(1:4793),sep="_")
families$Par1 <- as.character(families$Par1)
families$Par2 <- as.character(families$Par2)
Amat <- Amatrix(families, ploidy=2)
Amat <- createA(families)
families <- as.matrix(families)
families <- data.frame(c("a","b","c","d"), c("P1","P1","P2","P2"),c("P3","P4","P5","P6"))
